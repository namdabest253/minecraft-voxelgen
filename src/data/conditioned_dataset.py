...
- clip_embedding: [512] per-caption CLIP text embedding
- has_condition: bool — False when CFG dropout is applied

Loads precomputed RFSQ indices and per-caption CLIP text embeddings.
"""
...
class ConditionedLatentDataset(Dataset):
    ...
    Args:
        ...
        clip_embeddings_path: Path to caption_embeddings.pt
            (filename -> [512] embedding, plus "_null" key).
        split_filenames: Optional list of filenames to include.
    ...
    def __init__(
        ...
        # Load per-caption CLIP embeddings (filename -> [512] tensor)
        ...
        # Find available latent files that also have CLIP embeddings
        if split_filenames is not None:
            self.filenames = []
            for fn in split_filenames:
                stem = Path(fn).stem
                pt_path = self.latent_dir / f"{stem}.pt"
                if pt_path.exists() and fn in self.clip_embeddings:
            ...
            self.filenames = [
                f"{f.stem}.h5"
                for f in pt_files
                if f"{f.stem}.h5" in self.clip_embeddings
            ]

        n_latents = len(list(self.latent_dir.glob("*.pt")))
        n_embeddings = len(self.clip_embeddings) - 1  # exclude _null
        print(
            f"ConditionedLatentDataset: {len(self.filenames)} samples "
            f"(latents={n_latents}, embeddings={n_embeddings}, "
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        Returns:
            Dict with:
                ...
                "clip_embedding": [embed_dim] float32 — per-caption CLIP embedding
        ...
        # Per-caption CLIP embedding
        clip_emb = self.clip_embeddings[filename].clone()
